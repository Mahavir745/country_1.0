...
const fav_country_list = document.querySelector("#fav_country_list")
...
fav_icon.addEventListener("click",()=>{
  ...
  if(!found){
    fav_list.forEach((ele)=>{
      ...
      p3.textContent = "❤︎";
      p1.textContent = `${ele["flagName"]}`
      p2.textContent = ele["flagIcon"]

      div.append(p1,p2,p3);
      fav_country_list.appendChild(div);
    })
    found = true
  }
  fav_country_list.classList.toggle("fav_country_display")
})
...
function createCard(data){
  ...
  allcards.forEach((ele)=>{
    ele.addEventListener("click",()=>{
      ...
      localStorage.setItem("flagname", JSON.stringify(obj))
